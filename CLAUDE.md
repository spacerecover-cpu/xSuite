...
### Version 1.4.0 — Recovery Scenarios: Partial, Re-Recovery & No-Solution Follow-up
**Date**: 2026-07-04
**Migrations**: `case_status_manual_override` (free-form Overview status picker via `set_case_status`), `recovery_workflow_scenarios`

- **Free-form manual status override** (`set_case_status(uuid,uuid,text)` SECURITY DEFINER): the Case Detail → Overview Status picker sets any active status for any non-viewer staff with no sequence/evidence gates (still writes `case_job_history` `manual_override:true`; still notifies). The guided Stage Banner / clone-delivered / `log_case_checkout` keep the gated `transition_case_status`.
- **Partial recovery** is surfaced as a first-class **Outcome badge** (`OutcomeBadge`, full/partial/unrecoverable/declined) driven by `cases.recovery_outcome`; recording a recovery attempt rolls its result up to the case outcome (`caseQualityService`). Outcome stays DATA, never a status name.
