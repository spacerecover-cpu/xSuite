...
- **Status:** ✅ **FIXED** — `listPreviewRecords('report')` now returns each record's own subtype (`document_instances.report_subtype`, with the same `?? 'evaluation'` default `documentInstanceData.ts` applies, so the two layers cannot drift), and `TemplateStudio` resolves `builtIn` from that record subtype rather than the picker whenever a record is selected. The picker still drives SAMPLE previews, where there is no record to ask.
  **Made visible, not silent:** when the record overrules the picker the `<select>` is disabled with an explanatory `title`, shows the record's type as its value, and a "Type from record" chip appears — so the author sees *why* the control stopped responding instead of it quietly disagreeing with them.
  **Note on the column:** the value lives in `document_instances.report_subtype`, not a `report_type` column — the `report.report_type` name only exists in the fetch layer's mapped shape, which is what made the original finding's wording misleading.
  Covered by `reportSubtypeParity.test.ts`, which pins that the subtypes genuinely resolve different section sets (so choosing wrong is a real defect, not cosmetic) and that both layers share the `'evaluation'` null-default. It also records a behaviour found while testing: `reportConfigForSubtype` uppercases an *unknown* subtype straight into the document title rather than rejecting it — unreachable from a null column now that both defaults agree, but worth knowing.
...
- **Status:** ✅ **FIXED** — a "Start on a new page" checkbox now sits on each visible section in the Sections list (`OtherDetailsTab`), writing through the existing `api.patchSection`. It is deliberately hidden on the document's first visible section, where `renderTemplate` drops the flag anyway (a leading `pageBreak` opens pdfmake with a blank page) — offering a control that provably does nothing is its own defect.
  **A real bug surfaced while testing this.** The first fixture happened to target `meta`, which is the section that gets COMBINED with `parties` when `layout.partiesMetaSideBySide` is on — and the flag was silently dropped there, exactly as the correctness reviewer predicted. So the new checkbox would have appeared inert on that one section. `renderTemplate` now ORs `pageBreakBefore` across both members of the combined pair. Without the wiring test this would have shipped as "the checkbox doesn't work sometimes".
  Covered by `pageBreakWiring.test.ts`, which pins the whole chain the checkbox depends on — override → `resolveTemplateConfig` → rendered `pageBreak` — in both directions (ticking adds it, unticking removes it) plus a default-off parity case.
...
- **Status:** ✅ **FIXED** — took the report's second option (pin the ASCII runs) rather than shipping a patched font. `cellSoftWrapper` now returns pdfmake **runs** instead of a flat string when the document font cannot carry U+200B: the segments that actually received a break opportunity are emitted as `{ text, font: 'Roboto' }`, everything else stays in the document font. This is safe precisely because `wrapToken` only ever touches **pure-ASCII** tokens — no Arabic character can end up inside a pinned run, so shaping and cursive joining are untouched. Arabic/bilingual documents now wrap their long serials/hashes/URLs instead of keeping the overflow.
  **Parity:** a value that needed no wrapping still returns the identical plain string (no node churn), an English/Roboto document keeps the flat-string path unchanged, and `COPY_EXACT_COLUMN_KEYS` still short-circuits before any of this — so a device serial is never wrapped, pinned or otherwise touched, in any language.
  Covered in `dataTableRobustness.test.ts`: the previous test asserting Arabic documents produce **no** U+200B was replaced (it pinned the limitation as if it were the contract), and the new cases assert every pinned run carries a font that actually has the glyph, that no Arabic character is swept into one, and that the copy-exact carve-out holds under RTL too. Note the RTL cases locate cells by content rather than index — mirroring moves them.
