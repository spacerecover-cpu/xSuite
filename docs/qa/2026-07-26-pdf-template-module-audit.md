...
- **Status:** ✅ **FIXED** — the footer side inset now derives from the resolved paper margins instead of a flat 35pt. New `pageGeometry` exports `sideMargins()`, `footerSideInsets()`, `footerBlockMargin()` and the constant `FOOTER_OUTDENT`; `footerContentWidth()` was rewritten to subtract the **same** insets, so the divider rule can never desync from the block it is drawn in. `sections/footer.ts` replaced all six hardcoded `[35,0,35,25]` / `[35,10,35,25]` literals with `footerBlockMargin()`, and `sections/reportFooter.ts` replaced `[35,6,35,22]` the same way.
  **Design call, flagged for review:** the engine's built-in page margins are **40pt** while the footer inset has always been **35pt** — the footer already sits 5pt *outside* the text column. Aligning it flush would have moved the footer of every existing document by 5pt and dropped the default divider from 525 to 515: a parity break on the unconfigured A4 template, which this round's rules forbid. The inset therefore **tracks** the margins while preserving that 5pt outdent (`FOOTER_OUTDENT = 40 − 35`). Default → exactly 35 / 525; 20pt margins → 15pt inset; 60pt → 55pt. A flush footer is a one-line change (`FOOTER_OUTDENT = 0`) plus updated parity assertions, but it *is* a deliberate default-output change and should be an explicit decision, not a side effect.
...
- The `notification-dispatch-email` edge function carries its own copy of the renderer whose regex is `/\{\{(\w+)\}\}/` — it cannot match a pipe **or a dot**, so `{{invoice.total | currency}}` is delivered **literally** in the customer's inbox. `templateEngine.ts` documents this loudly in its module header and a test pins it, but nothing stops an admin typing a filter into `NotificationTemplatesTab`. Cheapest fix: a filter-detection warning in that editor's existing validate-and-confirm flow (added by the BIND-01 fix). `src/pages/settings/NotificationTemplatesTab.tsx` was outside this round's file set.
...
| **RND-03** footer margins ignore paper margins | ✅ Fixed — `footerBlockMargin()` / `footerSideInsets()`; 5pt `FOOTER_OUTDENT` preserved for parity (**design call, review it**) |
...
| **BIND-03** no number/currency/date formatting | ✅ Fixed — opt-in `{{key \| filter}}` + caller-supplied formatters. **No caller wired yet**; unsafe in notification templates → **FUP-03** |
