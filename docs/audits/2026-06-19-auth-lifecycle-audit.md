...
| ◑ **Backend done + verified; frontend cutover pending owner go-live** | **C3, C4** — re-audited 2026-06-20: the Option-A **backend is live and correct** — `portal` role + `authenticator` membership + grants, `get_current_portal_customer_id()`, `portal-login` edge fn (ACTIVE), and customer-scoped `TO portal` policies, **proven to isolate by SQL role/claim simulation**. **DB read-path + quote authz now complete & verified:** all portal-read tables have customer-scoped `TO portal` policies (`case_quotes` in `20260620051740`; `case_report_sections`, `case_quote_items`, `customer_communications`, `stock_sales`, `master_case_priorities` in `20260620053512`), and `approve_quote`/`reject_quote` are re-scoped to the portal customer (sim-verified no-op on another customer's quote). **Held:** `case_job_history` (raw internal audit log → needs a curated customer timeline, a product call). **Remaining:** (a) owner go-live — set edge `PORTAL_JWT_SECRET` + a test customer password (no customer has one, so E2E is currently impossible); (b) reconcile the `case_quotes`↔`quotes` id model (PortalQuotes passes a `case_quotes.id` to a `quotes`-updating RPC → approval non-functional today); (c) the **frontend** still uses the anon client + forgeable `sessionStorage` (cutover built + reverted, unsafe until (a)/(b)). Full map: `docs/portal-identity-design.md` §11–§12. | live DB + design doc |
