...
| ◑ **Backend built + verified; frontend cutover pending** | **C3, C4** — re-audited 2026-06-20: the Option-A **backend is already live and correct** — `portal` role + `authenticator` membership + grants, `get_current_portal_customer_id()`, `portal-login` edge fn (ACTIVE), and customer-scoped `TO portal` read policies, **proven to isolate by SQL role/claim simulation** (own rows only; forged/empty claim → 0). This PR adds the one missing read policy (`case_quotes`, migration `20260620051740`). **Still open:** (a) 6 more portal-read tables need `TO portal` policies; (b) `approve_quote`/`reject_quote` scope by tenant not customer (authz gap); (c) the **frontend** still uses the anon client + forgeable `sessionStorage` JSON (a prototype cutover was built and reverted — unsafe until (a)/(b) + go-live land); (d) go-live blockers: edge `PORTAL_JWT_SECRET` unset + 0 customer passwords (can't E2E-verify). Full map: `docs/portal-identity-design.md` §11–§12. | live DB + design doc |
