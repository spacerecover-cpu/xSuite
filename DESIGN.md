...
- **Decoration level:** Minimal-to-intentional. Typography and a tight token palette do the work; no decorative gradients, blobs, or texture. *(Scoped exception: the Cases "command center" KPI tiles use bold token-driven gradients — owner-approved 2026-06-24; see Decisions Log. Gradients are semantic-token only, so they re-theme per tenant and stay clear of the purple/hex bans.)*
