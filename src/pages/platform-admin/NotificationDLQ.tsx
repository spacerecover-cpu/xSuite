...
const StatusPills: React.FC<StatusPillsProps> = ({ event }) => {
  ...
  if (event.is_stuck) {
    pills.push(
      <span key="stuck" className="bg-danger-muted text-danger text-xs px-2 py-0.5 rounded-full font-medium flex items-center gap-1">
  ...
  if (event.is_unprocessed && !event.is_stuck) {
    pills.push(
      <span key="unproc" className="bg-warning-muted text-warning text-xs px-2 py-0.5 rounded-full font-medium">
        unprocessed
      </span>,
    );
  }
  if (event.failed_log_count > 0) {
    pills.push(
      <span key="failed" className="bg-danger-muted text-danger text-xs px-2 py-0.5 rounded-full font-medium">
  ...
  if (event.last_error) {
    pills.push(
      <span key="err" className="bg-danger-muted text-danger text-xs px-2 py-0.5 rounded-full font-medium">
        error
      </span>,
    );
  }
  if (pills.length === 0) {
    pills.push(
      <span key="ok" className="bg-slate-100 text-slate-400 text-xs px-2 py-0.5 rounded-full font-medium">
        ok
      </span>,
    );
  }
