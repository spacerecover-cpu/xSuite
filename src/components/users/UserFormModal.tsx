        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-5">
          ...
          {mode === 'edit' ? (
            <Input
              ...
              readOnly
              disabled
              hint="Email cannot be changed after user creation"
          ...
          {formData.role === 'technician' && (
            <SearchableSelect
              label="Case Access Level"
              floatingLabel
              shrinkDefaultValue
              usePortal
              value={formData.case_access_level || 'full'}
              onChange={(value) =>
                setFormData({ ...formData, case_access_level: value as 'restricted' | 'full' })
              }
              options={[
                { id: 'full', name: 'Full Access - Can view all cases' },
                { id: 'restricted', name: 'Restricted - Only own/assigned cases' },
              ]}
              hint={
