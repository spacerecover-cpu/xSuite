...
-- would abort the replay. Defer body validation (session-level SET: migration
-- runners execute files statement-by-statement or per-file transactions, and
-- SET LOCAL is a no-op outside a transaction block); prod already has this
